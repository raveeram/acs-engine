...
// HasClassicProfile returns true if the customer has a non-empty classic profile
func (p *Properties) HasClassicProfile() bool {
	for _, agentPoolProfile := range p.AgentPoolProfiles {
		if agentPoolProfile.ClassicProfile != "" {
			return true
		}
	}
	return false
